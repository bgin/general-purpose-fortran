...
static char ident[] = "@(#)Draws Newells Teapot as a heap of Bezier surface patches.";
...
int main( int argc, char *argv[] ){
   ...
   vogle_vinit(dev);          /* set up device */

   vogle_color(BLACK);
   vogle_clear();

   vogle_window(-5.0, 5.0, -5.0, 5.0, -5.0, 5.0);
   vogle_lookat(0.0, 0.0, 10.0, 0.0, 0.0, 0.0, 0.0);

   vogle_patchcurves(params[0], params[1]);
   vogle_patchprecision(params[2], params[3]);
   vogle_patchbasis(bezier, bezier);

   /*
    * Make an object with the teapot in it.
    */

   vogle_makeobj(TEAPOT);
      vogle_color(CYAN);
   ...
   while (1) {
#ifdef USE_MOUSE
      but = vogle_slocator(&sx, &sy);
      switch(but) {
      case 1:
         vogle_rotate(90.0 * sx, 'x');
         break;
      case 2:
         vogle_rotate(90.0 * sx, 'y');
         break;
      case 4:
         vogle_rotate(90.0 * sx, 'z');
         break;
      default:
         ;
      }
      vogle_pushmatrix();
         vogle_rotate(90.0 * sx, 'y');
         vogle_rotate(90.0 * sy, 'z');
         vogle_translate(0.0, 0.0, -1.5);
         vogle_color(BLACK);
         vogle_clear();
         vogle_callobj(TEAPOT);
                vogle_popmatrix();
      vogle_swapbuffers();

      switch (vogle_checkkey()) {
      case 'q':   /* Stop the program */
      case ESC:
         vogle_vexit();
         exit(0);
      case 'e':   /* Enlarge */
         vogle_scale(1.1, 1.1, 1.1);
         break;

      case 'r':   /* Reduce */
         vogle_scale(0.9, 0.9, 0.9);
         break;
