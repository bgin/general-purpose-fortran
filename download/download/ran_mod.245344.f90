...
! compare output using numdiff utility to template file.
      program testit
      implicit none
      integer                   :: iseed=-20111229
      doubleprecision,external  :: ran_mod
      integer                   :: idum
      integer                   :: i
      doubleprecision,parameter :: expected=5.875770471000937D-003
      integer                   :: j
      integer                   :: ireseed
      integer                   :: icount
      doubleprecision           :: t

      write(*,*)' Test of random number consistency for ran_mod(3f),SEED=',iseed

!     test 1: print first 1000 call results
      idum=ran_mod(iseed) ! note iseed is changed
      do i=1,1000
         write(*,101)i,ran_mod(iseed)
      enddo

! test 2: print every millionth call up to 100 million
      iseed=-20111229
      ireseed=ran_mod(iseed)
      icount=0
      do j=1,100
         do i = 1, 1000000
            t = ran_mod(iseed)
...
! test 3: should always be the same
      if (t .eq. expected ) then
         print *, "100 million calls to ran_mod() OK"
      else
         print *, "Fail. Expected ",expected," got ",t
      end if
101   format(" call ",i12," value ",g22.15 )
