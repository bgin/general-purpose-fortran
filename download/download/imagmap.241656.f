...
c       CALL METMAP(ISTVAL,NSX,NSY,NSX,NSY,0.,0.,XSIZE,YSIZE)
        ...
        CALL dl_translate(4.,4.)
        CALL METMAP2(ISTVAL,NSX,NSY,50,50,100,100,0.,0.,XSIZE,YSIZE)
C
C       NOW AXES PLOT TO ALL DEVICES
C
        CALL dl_color(1)                ! SET PLOT COLOR
C
        CALL dl_rect(0.,0.,XSIZE,YSIZE)
        AF=ANINT(XSIZE)+1.0101
        CALL DL_AXISC(0.,0.,'X',-1,XSIZE,0.,0.0,XSIZE,AF,0.12,6.2)
        CALL DL_AXISC(0.,YSIZE,'X',101,XSIZE,0.,0.0,XSIZE,AF,0.12,6.2)
        AF=ANINT(YSIZE)+1.0101
        CALL DL_AXISC(0.,0.,'Y',1,YSIZE,90.,0.0,YSIZE,AF,0.12,6.2)
        CALL DL_AXISC(XSIZE,0.,'Y',-101,YSIZE,90.,0.0,YSIZE,AF,0.12,6.2)
        CALL dl_flush()
C
...
C
        COMMON/DL_COM/MP,OX,OY,SF,PX,PY,MX,MW,MCOL,MSC,
     $          PANG,PVP(4)
        SAVE /DL_COM/
C
