...
! ierr=attrset(262144)
  ierr=addch(icharl(history(histline)(hpos:hpos)))
...
!   if (key > 0) write(*,*) 'key= ', key
    if (key == ascii_lf .or. key == ascii_cr .or. key==key_enter) then
      ...
      call showline(histline, hpos, history)
    else if (key == key_ic) then
      mode=3-mode
    else if (key == key_dc .or. key == ascii_bs) then
