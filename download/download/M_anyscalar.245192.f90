!===================================================================================================================================
program scalars
use M_anyscalar,only : anyscalar_to_real, int128, real256
...
integer(kind=int128)  :: hugest= huge(0_int128)
real(kind=real32)     :: rs= huge(0_real32)
real(kind=real64)     :: rm= huge(0_real64)
real(kind=real128)    :: rl= huge(0.0_real128)
real(kind=real256)    :: rh= huge(0.0_real256)

101 format(a10,"|",i39,"|",i11,"|",i39)
102 format(a10,"|",g0,t55,"|",i11,"|",g0)
   write(*,*)'First show the INTEGER variables of various KINDS we will be using'
   write(*,*) '  NAME   |VALUE                                  |KIND(VALUE)|10**RANGE(VALUE)'
   write(*,101) 'tiny   ',tiny,   kind(tiny),   10_int128**range(tiny)
   write(*,101) 'small  ',small,  kind(small),  10_int128**range(small)
   write(*,101) 'medium ',medium, kind(medium), 10_int128**range(medium)
   write(*,101) 'large  ',large,  kind(large),  10_int128**range(large)
   write(*,101) 'hugest ',hugest, kind(hugest), 10_int128**range(hugest)
   write(*,102) 'rs     ',rs,     kind(rs),     10.0_real256**range(rs)
   write(*,102) 'rm     ',rm,     kind(rm),     10.0_real256**range(rm)
   write(*,102) 'rl     ',rl,     kind(rl),     10.0_real256**range(rl)
   write(*,102) 'rh     ',rh,     kind(rh),     10.0_real256**range(rh)
...
202 format(a,*(g0:,'; '))
203 format(a,*(g0:,'; '))
   write(*,202,iostat=ios) 'SQUAREI()  :',squarei(tiny),  squarei(small), squarei(medium), squarei(large), squarei(hugest)
   write(*,203,iostat=ios) 'SQUAREI()  :',                squarei(rs),    squarei(rm),     squarei(rl),    squarei(rh)
...
real                 :: invalue_local
real                 :: dvalue
real,parameter       :: biggest=sqrt(real(0.0,kind=real256))
