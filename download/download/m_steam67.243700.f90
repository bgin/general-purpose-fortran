...
!@(#) compute thermal conductivity of water at pressure P and temperature T
!  ***********************************************************
!  ***********************************************************
!  **   FUNCTION CONDV67 WILL COMPUTE THE THERMAL CONDUCT-  **
!  **   IVITY OF WATER AT PRESSURE P AND TEMPERATURE T.     **
!  ***********************************************************
!  ***********************************************************
!  **                ARGUMENTS TO CONDV67                   **
!  **                                                       **
!  **   P       -  INPUT - WATER PRESSURE, IN PSIA.         **
!  **   T       -  INPUT - WATER TEMPERATURE, IN DEGREES F. **
!  **   CONDV67 - OUTPUT - CONDUCTIVITY IN BTU/HR-FT-DEG.F  **
!  ***********************************************************
!  ***********************************************************
...
! Copyright (C) 1969,1990 Westinghouse Electric Corporation.
!@(#) CPL67 calculates specific isobaric head capacity of water at pressure P and temperature T
      implicit double precision(A-H, O-Z)
...
!  Copyright (c) 1969,1990 Westinghouse Electric Corporation.
!@(#) function CPV67 calculates specific isobaric heat capacity and specific volume of water at pressure P and temperature T
   implicit double precision(A-H, O-Z)
   doubleprecision,intent(in)  :: pf    ! WATER PRESSURE, IN PSIA.
   doubleprecision,intent(in)  :: tf    ! WATER TEMPERATURE, IN DEGREES F.
...
!
!@(#) critical flow and degrees superheat of wet/superheated steam at PRES,ENTH
!       ***********************************************************
!       ***********************************************************
!       **                                                       **
!       **   FUNCTION crflo67 WILL CALCULATE THE CRITICAL FLOW   **
!       **   AND DEGREES SUPERHEAT OF WET OR SUPERHEATED STEAM   **
!       **   AT PRESSURE PRES AND ENTHALPY ENTH.                 **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
!       **                  ARGUMENTS TO crflo67                 **
!       **                                                       **
!       **     PRES  -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **     ENTH  -  INPUT - ENTHALPY, IN BTU / POUND.        **
!       **     SHT   - OUTPUT - DEGREES SUPERHEAT OF STEAM.      **
!       **     crflo67 - OUTPUT - CRITICAL FLOW LEVEL.           **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
      IMPLICIT double precision(A-H,O-Z)
...
! Copyright (C) 1969,1990 Westinghouse Electric Corporation.
!@(#) critical flow velocity and isentropic exponent of wet/superheated steam
!       ***********************************************************
!       **                                                       **
...
!       ***********************************************************
!       ***********************************************************
!       **                  ARGUMENTS TO crvel67                 **
!       **                                                       **
!       **   P       -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **   H       -  INPUT - ENTHALPY, IN BTU / POUND.        **
!       **   GA      - OUTPUT - ISENTROPIC EXPONENT.             **
!       **   crvel67 - OUTPUT - CRITICAL FLOW VELOCITY.          **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
...
! COPYRIGHT (C) 1969,1990 WESTINGHOUSE ELECTRIC CORPORATION. ALL RIGHTS
!@(#) calculate specific enthalpy/volume and entropy of liquid(P,T)
!       ***********************************************************
!       ***********************************************************
!       **                                                       **
...
!       **     P     -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **     T     -  INPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **     V     - OUTPUT - SPECIFIC VOLUME.                 **
!       **     S     - OUTPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **     IGO   -  INPUT - FLAG INDICATING CALLING ROUTINE. **
!       **                      (=1) HSL67.  (=2) SSL67.  (=3) VSL67.  **
!       **                      (=4) HCL67.  (=5) VCL.  (=6) DIR.  **
!       **     hcsl67  - OUTPUT - SPECIFIC ENTHALPY, IN BTU/LB.  **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
      IMPLICIT double precision(A-H,O-Z)
...
! Copyright (C) 1969,1990 Westinghouse Electric Corporation.
!@(#) calculate V,S,specific enthalpy for liquid given (P,T)
!       ***********************************************************
!       **                                                       **
...
!       **                      (=1) HSL67.  (=2) SSL67.  (=3) VSL67.  **
!       **                      (=4) HCL67.  (=5) VCL.  (=6) DIR.  **
!       **     hcslv167- OUTPUT - SPECIFIC ENTHALPY, IN BTU/LB.    **
!       **                                                       **
!       ***********************************************************
      implicit double precision(A-H, O-Z)
      !external STER67, GOTOER67
...
!       **                  ARGUMENTS TO vest67                  **
!       **                                                       **
!       **     PBARS -  INPUT - WATER PRESSURE, IN ATMOSPHERES.  **
!       **     TC    -  INPUT - WATER TEMPERATURE, IN DEGREES C. **
!       **     VOUT  - OUTPUT -                                  **
!       **                                                       **
!       ***********************************************************
...
! COPYRIGHT (C) 1969,1990 WESTINGHOUSE ELECTRIC CORPORATION. ALL RIGHTS
!@(#)   specific enthalpy = HSS67(P,T,s,v)
!       ***********************************************************
!       ***********************************************************
!       **                                                       **
!       **     FUNCTION HSS67 WILL CALCULATE SPECIFIC ENTHALPY,  **
!       **     SPECIFIC ENTROPY, AND SPECIFIC VOLUME OF WATER    **
...
!       **     S     - OUTPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **     V     - OUTPUT - SPECIFIC VOLUME.                 **
!       **     HSS67 - OUTPUT - SPECIFIC ENTHALPY, IN BTU/LB.    **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
      IMPLICIT double precision(A-H,O-Z)
...
!  **                   ARGUMENTS TO PSL67                  **
!  **     T     -  INPUT - WATER TEMPERATURE, IN DEGREES F. **
!  **     PSL67   - OUTPUT - SATURATION PRESSURE, IN PSIA.  **
!  ***********************************************************
   implicit double precision(A-H, O-Z)
   SAVE
   COMMON /NUST/TCORE, SCORE, VCORE, DPDT
...
!  **                  ARGUMENTS TO PSL167                  **
!  **     T     -  INPUT - WATER TEMPERATURE, IN DEGREES F. **
!  **     PSL167  - OUTPUT - SATURATION PRESSURE, IN PSIA.  **
!  ***********************************************************
   J = 2
   GOTO 1
!===================================================================================================================================
...
!       **                                                       **
!       **     SS    -  INPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **     PSV167  - OUTPUT - SATURATION PRESSURE, IN PSIA.  **
!       ***********************************************************
      J = 1
      GOTO 1
!#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#
...
!       ***********************************************************
!       **     SS    -  INPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **     PSV267  - OUTPUT - SATURATION PRESSURE, IN PSIA.  **
!       **                                                       **
!       ***********************************************************
      ENTRY PSV267(SS)
      J = 2
...
!       **                                                       **
!       **     SS    -  INPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **     PLS67   - OUTPUT - SATURATION PRESSURE, IN PSIA.  **
!       **                                                       **
!       ***********************************************************
...
!       ***********************************************************
!       ***********************************************************
!       **                 ARGUMENTS TO sssiss67                   **
!       **                                                       **
!       **     P     -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **     H     -  INPUT - SPECIFIC ENTHALPY, IN BTU/POUND. **
!       **     T     - OUTPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **     V     - OUTPUT - SPECIFIC VOLUME.                 **
!       **     X     - OUTPUT - FRACTION OF STEAM QUALITY.       **
!       **     sssiss67- OUTPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **                                                       **
!       ***********************************************************
      implicit double precision(A-H, O-Z)
...
!       **   V     - OUTPUT - SPECIFIC VOLUME.                   **
!       **   X     - OUTPUT - FRACTION OF STEAM QUALITY.         **
!       **   sisiss67- OUTPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **                                                       **
!       ***********************************************************
...
!       ***********************************************************
!       ***********************************************************
!       **                 ARGUMENTS TO hisiss67                 **
!       **                                                       **
!       **     P     -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **     S     -  INPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **     T     - OUTPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **     V     - OUTPUT - SPECIFIC VOLUME.                 **
!       **     X     - OUTPUT - FRACTION OF STEAM QUALITY.       **
!       **     hisiss67- OUTPUT - SPECIFIC ENTHALPY, IN BTU/LB.  **
!       **                                                       **
!       ***********************************************************
...
!       **     V     - OUTPUT - SPECIFIC VOLUME.                 **
!       **     X     - OUTPUT - FRACTION OF STEAM QUALITY.       **
!       **     hssiss67- OUTPUT - SPECIFIC ENTHALPY, IN BTU/LB.  **
!       **                                                       **
!       ***********************************************************
...
!       **     P     -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **     S     -  INPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!       **     TPS67   - OUTPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
      IMPLICIT double precision(A-H,O-Z)
...
!       ***********************************************************
!       **                                                       **
!       **     FUNCTION tsl67 WILL CALCULATE THE SATURATION TEMP-  **
!       **     ERATURE OF THE SATURATED LIQUID AT PRESSURE P.    **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
!       **                   ARGUMENTS TO tsl67                  **
!       **                                                       **
!       **     P     -  INPUT - SATURATION PRESSURE, IN PSIA.    **
!       **   tsl67   - OUTPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **                                                       **
!       ***********************************************************
      implicit double precision(A-H, O-Z)
      !external psl1
...
!       **                                                       **
!       **     P     -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **   tsl167  - OUNPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **                                                       **
!       ***********************************************************
      K = 2
1     CONTINUE
...
!       ***********************************************************
!       **                                                       **
!       **     FUNCTION tslh67 WILL CALCULATE SATURATION TEMPER- **
!       **     ATURE OF THE SATURATED LIQUID AT ENTHALPY ENTH.   **
!       **                                                       **
!       ***********************************************************
!       ***********************************************************
!       **                   ARGUMENTS TO tslh67                 **
!       **                                                       **
!       **   ENTH  -  INPUT - SPECIFIC ENTHALPY, IN BTU/LB.      **
!       **   tslh67  - OUTPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **                                                       **
!       ***********************************************************
      implicit double precision(A-H, O-Z)
      !external HSL67
...
!       **     P     -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **     T     -  INPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **     visl67  - OUTPUT - VISCOSITY, IN LB/FT-SEC.       **
!       **                                                       **
!       ***********************************************************
      implicit double precision(A-H, O-Z)
      !external VCL67
...
!       **     P     -  INPUT - WATER PRESSURE, IN PSIA.         **
!       **     T     -  INPUT - WATER TEMPERATURE, IN DEGREES F. **
!       **     visv67  - OUTPUT - VISCOSITY, IN LB/FT-SEC.       **
!       **                                                       **
!       ***********************************************************
...
! Copyright (C) 1969,1990 Westinghouse Electric Corporation.
!@(#) calculate specific enthalpy and entropy of water at pressure P and temp T
!  ***********************************************************
...
!  **     S     - OUTPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!  **     V     - OUTPUT - SPECIFIC VOLUME.                 **
!  **     hsv67   - OUTPUT - SPECIFIC ENTHALPY, IN BTU/LB.  **
!  **                                                       **
!  ***********************************************************
   implicit doubleprecision(A-H, O-Z)
   SAVE
...
!  ***********************************************************
!  ***********************************************************
!  **                  ARGUMENTS TO prstm67                 **
!  **                                                       **
!  **   P     -  INPUT - WATER PRESSURE, IN PSIA.           **
!  **   T     -  INPUT - WATER TEMPERATURE, IN DEGREES F.   **
!  **   prstm67 - OUTPUT - PRANDTL NUMBER (DIMENSIONLESS)   **
!  **                                                       **
!  ***********************************************************
   implicit doubleprecision(A-H, O-Z)
...
!  ***********************************************************
!  ***********************************************************
!  **                 ARGUMENTS TO sssicl67                 **
!  **                                                       **
!  **   P       -  INPUT - WATER PRESSURE, IN PSIA.         **
!  **   H       -  INPUT - SPECIFIC ENTHALPY, IN BTU/POUND. **
!  **   T       - OUTPUT - WATER TEMPERATURE, IN DEGREES F. **
!  **   sssicl67- OUTPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!  ***********************************************************
   implicit doubleprecision(A-H, O-Z)
   SAVE
   COMMON /NUST/TCORE, SCORE, VCORE, DPDT
...
!  ***********************************************************
!  ***********************************************************
!  **                  ARGUMENTS TO hssicl67                **
!  **                                                       **
!  **     PRES  -  INPUT - WATER PRESSURE, IN PSIA.         **
!  **     S     -  INPUT - SPECIFIC ENTROPY, IN BTU/LB-DEGF **
!  **     TEMP  - OUTPUT - TEMPERATURE, IN DEGREES F.       **
!  **     hssicl67- OUTPUT - ENTHALPY, IN BTU/POUND.        **
!  ***********************************************************
   p = pres
   FACTOR = 0.0d0
   T = TPSL67(P, S)
...
!       ***********************************************************
      ...
      SAVE
!-ju  dimension t(2)
!-ju  highest subscript needed is (2*n)
      integer i,k,n
      DIMENSION T(2*N)
!#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#
      DO I = 2, N
...
!       **                ARGUMENTS TO p23t67                    **
!       **                                                       **
!       **   T       -  INPUT - TEMPERATURE, IN DEGREES F.       **
!       **   p23t67  - OUTPUT - PRESSURE AT BOUNDARY, IN PSIA.   **
!       **                                                       **
!       ***********************************************************
...
!       **                  ARGUMENTS TO vliq67                  **
!       **                                                       **
!       **     PIN   -  INPUT - WATER PRESSURE, IN ATMOSPHERES.  **
!       **     TIN   -  INPUT - WATER TEMPERATURE, IN DEGREES C. **
!       **     VMIN  - OUTPUT -                                  **
!       **     VMAX  - OUTPUT -                                  **
!       **     vliq67  - OUTPUT -                                **
!       **                                                       **
...
!       ***********************************************************
!       **                                                       **
!       **         H2 = ZSDH67(P1,P2,H1,S,T1,T2,X1,X2,V1,V2)     **
!       **                                                       **
!       **   P1  INLET PRESSURE IN PSIA                   INPUT  **
!       **   P2  EXIT PRESSURE IN PSIA                    INPUT  **
!       **   H1  INLET ENTHALPY IN BTU/LB.                INPUT  **
!       **   H2  EXIT ENTHALPY IN BTU/LB.                 OUTPUT **
!       **   S   ENTROPY IN BTU/LB.-DEGREES F.            OUTPUT **
!       **   T1  INLET TEMPERATURE IN DEGREES F.          OUTPUT **
!       **   T2  EXIT TEMPERATURE IN DEGREES F.           OUTPUT **
!       **   X1  INLET QUALITY                            OUTPUT **
!       **   X2  EXIT QUALITY                             OUTPUT **
!       **   V1  INLET SPECIFIC VOLUME IN CUBIC FEET/LB.  OUTPUT **
!       **   V2  EXIT SPECIFIC VOLUME IN CUBIC FEET/LB.   OUTPUT **
!       **                                                       **
!       ***********************************************************
...
!       ***********************************************************
!       **                                                       **
!       **         H2 = zsdt67(P1,P2,T1,S,H1,T2,X1,X2,V1,V2)     **
!       **                                                       **
!       **   P1  INLET PRESSURE IN PSIA                   INPUT  **
!       **   P2  EXIT PRESSURE IN PSIA                    INPUT  **
!       **   T1  INLET TEMPERATURE IN DEGREES F.          INPUT  **
!       **   H2  EXIT ENTHALPY IN BTU/LB.                 OUTPUT **
!       **   S   ENTROPY IN BTU/LB.-DEGREES F.            OUTPUT **
!       **   H1  INLET ENTHALPY IN BTU/LB.                OUTPUT **
!       **   T2  EXIT TEMPERATURE IN DEGREES F.           OUTPUT **
!       **   X1  INLET QUALITY                            OUTPUT **
!       **   X2  EXIT QUALITY                             OUTPUT **
!       **   V1  INLET SPECIFIC VOLUME IN CUBIC FEET/LB.  OUTPUT **
...
!     to standard error on unicos, to logfile on cos
      ...
      INTEGER I
      WRITE (STRING, 110) NAME(:MAX(LEN(NAME), 70)), I
      CALL ZQJREM67(' STEAM TABLE ERROR')
      CALL ZQJREM67(STRING)
      STOP
110   FORMAT (' ** ERROR IN ', A, I5)
      END
!#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#
...
!  **                  ARGUMENTS TO STER67                  **
!  **                                                       **
!  **     NAME  -  INPUT - NAME OF THE CALLING ROUTINE.     **
!  **     I     -  INPUT - FLAG FOR DATA TO BE PRINTED.     **
!  **     A     -  INPUT - DATUM ITEM 1 TO BE PRINTED.      **
!  **     B     -  INPUT - DATUM ITEM 2 TO BE PRINTED.      **
!  ***********************************************************
!  ***********************************************************
   implicit double precision(A-H, O-Z)
...
!#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#
      PROGRAM ASME
!@(#) Calculate steam table properties from a small table-driven program
      use m_steam67
      IMPLICIT doubleprecision (A-H,O-Z)
!...PROGRAM WIDE VARIABLE
      character(len=20) :: version
      character(len=2)  :: C1,C2,CC
      character(len=3)  :: readc
!...SATURATION VARIABLES AND FORMATS
      REAL SATPROP(17)
      COMMON /SATPRPS/ PSAT,TSAT,HFG,HSATL,HSATV,VL,VV,CL,CV,SL,SV,AML,AMV,AKL,AKV,PL,PV
      EQUIVALENCE (SATPROP(1),PSAT)
      CHARACTER(len=50) ::  SATOUT(2,17)

!...NON-SATURATION VARIABLES AND FORMATS
      LOGICAL LBRK13,LBRK15
      doubleprecision    NSATPRP(4)
      COMMON /NSATPRP/TSC,P,H,T
      EQUIVALENCE (NSATPRP(1),TSC)
      CHARACTER(len=40) ::  NSATOUT(2,4)

!...ERROR MESSAGE ARRAY DECLARATIONS
      CHARACTER(len=50) ::  ERRMSG(7)
      INTEGER LMSG(7)
...
!...SATURATION OUTPUT FORMAT DATA STATEMENTS
      DATA SATOUT /     &
     &'Saturation Pressure........',' psia       ',     &
     &'Saturation Temperature.....',' deg F   ',     &
     &'Heat of Vaporization.......',' Btu/lb     ',     &
     &'Liquid Side Enthalpy.......',' Btu/lb   ',     &
     &'Vapor Side Enthalpy........',' Btu/lb     ',     &
     &'Liquid Specific Volume.....',' ft3/lb     ',     &
     &'Vapor Specific Volume......',' ft3/lb     ',     &
     &'Liquid Specific Heat.......',' Btu/lb-F   ',     &
     &'Vapor Specific Heat........',' Btu/lb-F   ',     &
     &'Liquid Entropy.............',' Btu/lb-F   ',     &
     &'Vapor Entropy..............',' Btu/lb-F   ',     &
     &'Liquid Viscosity...........',' lb/ft-sec  ',     &
     &'Vapor Viscosity............',' lb/ft-sec  ',     &
     &'Liquid Thermal Cond........',' Btu/hr-ft-F',     &
     &'Vapor Thermal Cond.........',' Btu/hr-ft-F',     &
     &'Liquid Prandtl Number......','            ',     &
     &'Vapor Prandtl Number.......','            '/

!...NON-SATURATION OUTPUT FORMAT DATA STATEMENTS
      DATA NSATOUT/     &
     &'Subcooling.................',' deg F      ',     &
     &'Pressure...................',' psia       ',     &
     &'Enthalpy...................',' Btu/lb     ',     &
...
!...ERROR MESSAGES DATA
      DATA LMSG/20,34,41,43,45,24,24/
      DATA ERRMSG(1)/'BAD VALUE, TRY AGAIN'/
      DATA ERRMSG(2)/'VALUE MUST BE .GT. 0 AND .LT. 906.'/
      DATA ERRMSG(3)/'VALUE MUST BE .GT. 32.02 AND .LT. 705.47.'/
      DATA ERRMSG(4)/'VALUE MUST BE .GT. 0.0886 AND .LE. 3208.234'/
      DATA ERRMSG(5)/'VALUE MUST BE .GT. 0.0886 AND .LE. 14695.9488'/
      DATA ERRMSG(6)/'VALUE MUST BE BETWEEN '/
      DATA ERRMSG(7)/'VALUE MUST BE BETWEEN '/

!...STATEMENT FUNCTIONS
      LSAT(I,J) = LSTRING4(SATOUT(I,J))
      LNST(I,J) = LSTRING4(NSATOUT(I,J))

!...PROMPT FOR DESIRED FUNCTION
      CALL STEAMV67(6,version)
 2    IERR = 1
      WRITE(6,2001)
 2001 FORMAT(/,' Known Properties -',/,/,           &
     &         ' 1. Saturation Pressure',/,         &
     &         ' 2. Saturation Temperature',/,      &
     &         ' 3. Liquid Sat. Enthalpy',/,        &
     &         ' 4. Pressure and Enthalpy',/,       &
     &         ' 5. Pressure and Temperature',/,    &
     &         ' Select by Number or q to quit: ')
      READ(5,'(a)',END=3001,ERR=3001) READC
      IF(READC.EQ.'Q')STOP
      IF(READC.EQ.'q')STOP
      READ(READC,'(i3)',END=3001,ERR=3001) IC
      IF(IC.LT.1.OR.IC.GT.5) GO TO 3001

!...PROGRAM FLOW VIA COMPUTED GO TO
      WRITE(6,'(A)') ' '
      GO TO (70,40,20,90,90) IC

!...INPUT ERROR PROCESSING
3001  LERR = LMSG(IERR)
      WRITE(6,3002) ERRMSG(IERR)(:LERR)
      GO TO (2,20,50,75,95,111,151) IERR
3002  FORMAT(/,1X,A)
...
! CALCULATE VALUES BASED UPON A SPECIFIED SATURATED LIQUID ENTHALPY
   20 IERR = 2
      WRITE(6,2003) SATOUT(1,4)(:LSAT(1,4)),SATOUT(2,4)(:LSAT(2,4))
 2003 FORMAT(1X,'Enter ',A,A,': ')
      READ(5,*,END=3001,ERR=3001) HSATL
      IF(HSATL.LE.0.D0.OR. HSATL.GT.906.D0) GO TO 3001
 2011 FORMAT('  Input Value   ',A,A,1PG14.6,/)
      TSAT=TSLH67(HSATL)
   31 PSAT=PSL67(TSAT)
   32 HSATV=HSV67(PSAT,T,S,V)
      SL=SSL67(TSAT)
      VL=VSL67(TSAT)
      CV=CPV67(PSAT,TSAT,VV)
      CL=CPL67(PSAT,TSAT)
      AKL=CONDL67(PSAT,TSAT)
      AKV=CONDV67(PSAT,TSAT)
      SV=SSSISS67(PSAT,HSATV,T,V,X)
      AML=VISL67(PSAT,TSAT)
      AMV=VISV67(PSAT,TSAT)
      PL=PRLIQ67(PSAT,TSAT)
      PV=PRSTM67(PSAT,TSAT)
      HFG=HSATV-HSATL

!...OUTPUT RESULTS
2010  FORMAT(1X,A2,A27,1PG14.6,A12)
2020  FORMAT(1X,A,A,1PG14.6,A)
      DO ILIST=1,17
         WRITE(6,2010) C1,SATOUT(1,ILIST),SATPROP(ILIST),SATOUT(2,ILIST)
      enddo
      CALL RESUME(*2)

! CALCULATE VALUES BASED ON SATURATED TEMPERATURE
   40 IERR = 3
   50 WRITE(6,2003) SATOUT(1,2)(:LSAT(1,2)),SATOUT(2,2)(:LSAT(2,2))
      READ(5,*,END=3001,ERR=3001)TSAT
      IF(TSAT.LT.32.02D0.OR.TSAT.GE.705.47D0) GO TO 3001
   60 HSATL=HSL67(TSAT)
      GO TO 31

! CALCULATE VALUES BASED ON SATURATED PRESSURE
   70 IERR = 4
   75 WRITE(6,2003) SATOUT(1,1)(:LSAT(1,1)),SATOUT(2,1)(:LSAT(2,1))
      READ(5,*,END=3001,ERR=3001)PSAT
      IF(PSAT.LE.0.0886D0.OR. PSAT.GT.3208.234D0) GO TO 3001
      TSAT=TSL67(PSAT)
      HSATL=HSL67(TSAT)
      GO TO 32

! CALCULATE VALUES BASED ON PRESSURE AND ENTHALPY/TEMPERATURE   IC = 4/5
   90 IERR = 5
   95 WRITE(6,2003) NSATOUT(1,2)(:LNST(1,2)),NSATOUT(2,2)(:LNST(2,2))
      READ(5,*,END=3001,ERR=3001)P
      IF(P.LE.0.0886D0.OR.P.GT.14695.9488D0)GO TO 3001
      IF(IC.EQ.5)GO TO 150
      IERR = 6
  110 WRITE(6,2003) NSATOUT(1,3)(:LNST(1,3)),NSATOUT(2,3)(:LNST(2,3))
      READ(5,*,END=3001,ERR=3001)H
      HMAX=HSS67(P,1472.D0,SX,VX)
      HMIN=0.0
      IF(P.GT.3208.234D0)HMIN=HSS67(P,705.47D0,SX,VX)
      IF(H.GT.HMIN.AND.H.LE.HMAX)GO TO 120
      GO TO 3001
  111 WRITE(6,'(1X,F10.3,'' AND '',F10.3)') HMIN,HMAX
      GO TO 110
  120 IF(P.GT.3208.234D0)GO TO 140
      HSATV=HSV67(P,TSAT,XX1,XX2)
      HSATL=HSL67(TSAT)
      IF(H.LT.HSATL)GO TO 130
      IF(H.GT.HSATV)GO TO 140
      S=SSSISS67(P,H,T,V,X)
      WRITE(6,2005) C1,X,V
 2005 FORMAT(1X,A,'Saturated with a Quality of',F5.3,' and Specific Volume of',1PG14.6)
      WRITE(6,2006) C1,P,H,T
 2006 FORMAT(1X,A,'Pressure =',F10.3,' Enthalpy=',F10.3,' Temperature=',F10.3)
      GO TO 31

! CALCULATE SUBCOOLED VALUES
  130 SL=SSSICL67(P,H,T)
  131 VL=VCL67(P,T)
      AML=VISL67(P,T)
      PL=PRLIQ67(P,T)
      CL=CPL67(P,T)
      AKL=CONDL67(P,T)
      TSC=TSAT-T
      NSATOUT(1,1)='Subcooling.................'
      WRITE(6,2010)                                                                                          &
     & (C1,NSATOUT(1,ILIST)(:LNST(1,ILIST)),NSATPRP(ILIST),NSATOUT(2,ILIST)(:LNST(2,ILIST)),ILIST=1,4),      &
     & (C1,SATOUT(1,JLIST)(:LSAT(1,JLIST)) ,SATPROP(JLIST),SATOUT(2,JLIST)(:LSAT(2,JLIST)), JLIST=6,16,2)
      CALL RESUME(*2)

! CALCULATE SUPERHEATED VALUES
  140 SV=SSSISS67(P,H,T,VV,XX5)
  141 ISTA = 2
      CV=CPV67(P,T,VV1)
      IF(P.GE.11756.75924D0) GO TO 145
      AMV=VISV67(P,T)
      IF(P.GE.7347.9744D0)GO TO 145
      PV=PRSTM67(P,T)
      AKV=CONDV67(P,T)
      IF(P.GT.3208.234D0)GO TO 145
      TSC=T-TSAT
      ISTA = ISTA - 1
      NSATOUT(1,1)='Superheating...............'
  145 CONTINUE
      WRITE(6,2010) (C1,NSATOUT(1,ILIST)(:LNST(1,ILIST)),           &
     &                  NSATPRP(ILIST),                             &
     &                  NSATOUT(2,ILIST)(:LNST(2,ILIST)),           &
     &                  ILIST=ISTA,4)

      LBRK13=P.GT.11756.75924
      LBRK15=P.GT.7347.9744

      DO 146 JLIST=7,17,2
      IF((JLIST.EQ.13.AND.LBRK13).OR.(JLIST.EQ.15.AND.LBRK15)) GO TO 147
      WRITE(6,2020)C1,SATOUT(1,JLIST)(:LSAT(1,JLIST)),SATPROP(JLIST),SATOUT(2,JLIST)(:LSAT(2,JLIST))
146   CONTINUE
147   CALL RESUME(*2)

! CALCULATE PROPERTIES BASED ON PRESS. AND TEMP.  IC = 5 (TEMP.)
  150 IERR = 7
      WRITE(6,2003) NSATOUT(1,4)(:LNST(1,4)),NSATOUT(2,4)(:LNST(2,4))
      READ(5,*,END=3001,ERR=3001)T
      TMIN=32.02
      TMAX=1472.
      IF(P.GT.3208.234D0)TMIN=705.47
      IF(T.GE.TMIN.AND.T.LE.TMAX)GO TO 155
      GO TO 3001
  151 WRITE(6,'(1X,F10.3,'' AND '',F10.3)') TMIN,TMAX
      GO TO 150
  155 IF(P.GT.3208.234D0)GO TO 158
      TSAT=TSL67(P)
      IF(T.EQ.TSAT)GO TO 60
      IF(T.LT.TSAT)GO TO 160
  158 H=HSS67(P,T,SV,VV)
      GO TO 141
  160 H=HCL67(P,T,SL)
      GO TO 131
      END
      ...
      WRITE(6,1)
 1    FORMAT(/,1X,'Press RETURN key to CONTINUE ')
      READ(5,'(A)',END=2,ERR=2) JUNK
 2    WRITE(6,'(A)') ' '
      RETURN 1
      END
      FUNCTION LSTRING4(CHAR)
      CHARACTER(len=*) :: CHAR
      INTEGER LTOT
!
      LTOT=LEN(CHAR)
      DO 1 I=LTOT,1,-1
      IF(CHAR(I:I).NE.' ') GO TO 10
1     CONTINUE
      I=0
10    LSTRING4 = I
      RETURN
