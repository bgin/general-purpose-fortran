...
use M_debug, only : unit_check, unit_check_good, unit_check_done, unit_check_start, unit_check_level, msg
implicit none
...
integer                     :: i
   ...
   call unit_check('least_common_multiple',least_common_multiple(array).eq.answer,msg=msg(message,line, ']) the value is',answer))
end subroutine writeit_v
...
integer, allocatable :: matrix(:,:)
   ...
   character(len=1024) :: line2
      ...
      call unit_check('greatest_common_divisor',greatest_common_divisor(vector).eq.answer,msg=msg(line1,line2))
   end subroutine writeit_v
   ...
   character(len=1024) :: line3
      ...
      call unit_check('greatest_common_divisor',greatest_common_divisor(matrix).eq.answer,msg=msg(line1,line2,line3))
   end subroutine write_matrix
...
integer,parameter :: nn(*)=[      9973, 1127326861, 1127326883, 1127326897, 1127326901, 1127326927, 1127326957, &
   ...
   CHECKTHEM: block
   do i=1,size(nn)
      call unit_check('i_is_prime',i_is_prime(nn(i)),msg=msg('testing',nn(i),'which should be prime'))
   enddo
   endblock CHECKTHEM
...
integer,allocatable :: exponents(:)
   ...
   exponents=[1,1,1]
   call prime_factors(number, nprm, iprm, iexp,verbose)
   call unit_check('prime_factors',all(iprm(:nprm).eq.primes).and.all(iexp(:nprm).eq.exponents),msg=msg('factors of ',number))
