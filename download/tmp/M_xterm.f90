...
character(len=*),intent(in)   :: windowname
   ...
   write(*,'(a)')esc//']3;WM_OCCUPATION='//trim(windowname)//bel
   write(*,'(a)')esc//']3;WM_CURRENTWORKSPACE='//trim(windowname)//bel
...
character(len=*),intent(in) :: color
   ...
   write(*,'(a,i0,a)',advance='no') esc//']4;', pennum, ';'//trim(color)//bel
...
character(len=*),parameter::ident_7="@(#)M_xterm::xterm_font(3f): set a font by name in an xterm window"
   ...
   case('-','+','0':'9') ; write(*,'(a)',advance='no')esc//"]50;#"//trim(fontname)//bel   ! try the string that was found
   case('up')            ; write(*,'(a)',advance='no')esc//"]50;#+"//bel                  ! up list of predefined fonts
   case('down')          ; write(*,'(a)',advance='no')esc//"]50;#-"//bel                  ! down list of predefined fonts

   case('default')       ; write(*,'(a)',advance='no')esc//"]50;#0"//bel
   case('unreadable')    ; write(*,'(a)',advance='no')esc//"]50;#1"//bel
   case('tiny')          ; write(*,'(a)',advance='no')esc//"]50;#2"//bel
   case('small')         ; write(*,'(a)',advance='no')esc//"]50;#3"//bel
   case('medium')        ; write(*,'(a)',advance='no')esc//"]50;#4"//bel
   case('large')         ; write(*,'(a)',advance='no')esc//"]50;#5"//bel
   case('huge')          ; write(*,'(a)',advance='no')esc//"]50;#6"//bel
   case('escape')        ; write(*,'(a)',advance='no')esc//"]50;#7"//bel
   case('selection')     ; write(*,'(a)',advance='no')esc//"]50;#8"//bel

   case('')              ; write(*,'(a)',advance='no')esc//"]50;fixed"//bel  ! the standard default X11 Windows font name

   case default
      write(*,'(a)',advance='no')esc//"]50;"//trim(fontname)//bel   ! try the string that was found
   end select
...
case default                                  ! unknown keyword
   write(ERROR_UNIT,'(a,a)')  '*xterm_keywords* : unknown keyword:',trim(keyword)
   return
end select
...
if(present(right).and.present(down))then
   ...
   if(present(right))then
      write(*,'(a,i0,a,i0,a)',advance='no') CSI//'3;',right,        ';',current_down,'t'
   elseif(present(down))then
      write(*,'(a,i0,a,i0,a)',advance='no') CSI//'3;',current_right,';',down,        't'
   endif
endif
...
elseif(present(rows))then
   write(*,'(a,i0,a,i0,a)',advance='no') CSI//'8;',rows,';t'
elseif(present(cols))then
   write(*,'(a,i0,a,i0,a)',advance='no') CSI//'8;;',cols,'t'
endif
...
&@(#)M_xterm::xterm_width(3f): set size of xterm(1) window to standard sizes using escape sequences"
   ...
   select case(iwidth)
   case(80)
      write(*,'(a)',advance='no') CSI//'?3l' ! 80 was specified
   case(132)
      write(*,'(a)',advance='no') CSI//'?3h' ! 132 was specified
   case default
      return
   end select
...
select case(type)
case('title');     write(*,'(a)',advance='no') esc//']2;'//label//bel
case('name');      write(*,'(a)',advance='no') esc//']1;'//label//bel
case('nt');        write(*,'(a)',advance='no') esc//']0;'//label//bel
case default
   write(ERROR_UNIT,'(a,a)')  '*xterm_labels* : unknown label type:',trim(type)
   return
end select
