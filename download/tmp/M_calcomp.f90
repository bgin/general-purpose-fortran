...
!!
!!    Because I have had need to quickly resurrect or interface with codes
...
!!    TEMPLATE. PDF files were a proprietary metafile which was prepared
!!    for specific output devices using TEMPLATE routines that converted
!!    PDFs to any of the TEMPLATE output devices (which probably ran to
!!    well over a hundred formats). Note that these file have nothing to
!!    do with Adobe Acrobat PDF files. Unless you somehow still have a
!!    TEMPLATE license, this code is of historical interest only.
!!
...
!!    original CALCOMP library except for a few minor changes made originally to
!!    call the commercial graphics library "TEMPLATE" (instead of generating a
!!    file of CALCOMP plotter commands), as noted in the documentation.
!!
!!    Note that the most common point of divergence between "CALCOMP-compatible"
!!    libraries was whether a string was a packed integer array of various word
!!    sizes (ie. "Hollerith") or an array of characters or a character string.
!!
!!##HISTORY
!!
!!     Originally the CALCOMP library was a simple graphics library that created
!!     files composed of instructions that drove a very popular family of pen
!!     plotters made by CALCOMP. The library was supplied by the manufacturer.
!!     This made it easier for CALCOMP customers to create plots on these old
!!     vector-based monochrome (then color) pen plotters using Fortran 66 or 77
!!     (which was the most popular programming language for graphics). For the
!!     most part, codes generated plots not by writing specific standardized
...
!!         assumed "a CALCOMP-compatible" library was available for customers to
!!         hook their package to.
!!       + It was assumed a programmer was available to interface the plotter to
!!         codes and utilities that needed to use it.
!!       + Almost every plotter vendor provided or sold a "CALCOMP-compatible"
...
!
      ...
      SAVE DX,DY,PENX,PENY,XLAST,XOFF,XORG,YLAST,YORG,YS
!
...
!  IS CALLED, BUT ONLY AT THE BEGINNING OF THE PROGRAM; THEREFORE,
!  A DATA STATEMENT WAS USED TO PERFORM THE INITIALIZATION PROCESS.
!
...
!
!  THE VARIABLE 'PI' CONTAINS THE VALUE OF THE MATHEMATICAL CONSTANT
!  PI (TO THREE SIGNIFICANT PLACES).
!
      SAVE PI,XRLORG,YRLORG
...
!
!     Colaboration was obtained from Plimley(CA), Rowe(FL), and Butler
