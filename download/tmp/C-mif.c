...
static int MIF_header(void) {
   ...
   struct passwd *pw;
        fprintf(fp,"<Comment -\n");
        fprintf(fp,"  Creator: M_DRAW MIF driver\n");
        fprintf(fp,"  Title: Very Ordinary Graphics Learning Environment\n");
        ...
        fprintf(fp,"> # End of <Comment> Statement\n");
        return(0);
}
/******************************************************************************/
/* change index i in the color map to the appropriate rgb value */
int MIF_mapcolor(int i, int r, int g, int b){
    if(i >= CMAPSIZE || i < 0){
       return(-1);
    }
...
*/
   ...
   Y=Y-B;
   fprintf(fp,"<Color ");
   fprintf(fp,"<ColorTag `%3.3d%3.3d%3.3d%3.3d'>",C,M,Y,B);
   fprintf(fp,"<ColorCyan %d>",C);
   fprintf(fp,"<ColorMagenta %d>",M);
   fprintf(fp,"<ColorYellow %d>",Y);
   fprintf(fp,"<ColorBlack %d>",B);
   fprintf(fp,">");
   fprintf(fp,"<ObColor `%3.3d%3.3d%3.3d%3.3d'>",C,M,Y,B);
}
/******************************************************************************/
...
static int MIF_init(void) {
   ...
   fprintf(fp,"<MIFFile %d.0> # Generated by M_DRAW driver (John S. Urban, Sept. 1993)\n",MIFVERSION);
   fprintf(fp,"<Document\n");
   fprintf(fp,"<Units Umm>\n");
   fprintf(fp,"<DPageSize %f %f >\n",MIFXSIZE0,MIFYSIZE0);
   fprintf(fp,"># end of Document\n");

   MIF_header();
   fprintf(fp,"# MACRO section to reduce file size \n");
   fprintf(fp,"define(P,Point)\n");
   fprintf(fp,"define(S,Separation)\n");
   fprintf(fp,"define(SN,Smoothed No)\n");
   fprintf(fp,"define(PW,PenWidth)\n");
   fprintf(fp,"define(PL,PolyLine)\n");
   fprintf(fp,"define(F,Fill)\n");
   fprintf(fp,"define(GID,GroupID)\n");
   fprintf(fp,"define(G,Group)\n");
   fprintf(fp,"# END MACRO section to reduce file size \n");

   fprintf(fp,"<Page \n");
   fprintf(fp,"<PageSize %f %f >\n",MIFXSIZE0,MIFYSIZE0);
   ...
   fprintf(fp,"# ADDRESSABILITY IN RASTERS %d %d\n",vdevice.sizeSx,vdevice.sizeSy);
...
static int MIF_string(char *s) {
   ...
   members++;
   fprintf(fp, " <F %d >\n",curpat);
   fprintf(fp, " <TLOrigin    %f  %f > \n",realx0, realy0);
   kludgex=realx0;
   kludgey=realy0;
   fprintf(fp,"  <Font \n");
   fprintf(fp,"   <FFamily `Courier'> \n");
   fprintf(fp,"   <FSize %f > \n",vdevice.hheight/MIFTORAS);
   fprintf(fp,"   <FBold Yes> \n");
   fprintf(fp,"   <FUnderline  No> \n");
   fprintf(fp,"   <FSeparation %d > \n",curcol);
   fprintf(fp,"  > # end Font \n");
