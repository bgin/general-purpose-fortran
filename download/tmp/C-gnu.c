...
static int gnu_init(void) {
        ...
        fprintf(fp,"#PLOT 2\n");
        fprintf(fp,"# version 2.4.1 GNU plotutils portable(ASCII) metatfile subset generated by M_DRAW\n");
        fprintf(fp,"# meanings:\n");
        fprintf(fp,"# one command per line, values are space delimited.\n");
        fprintf(fp,"#  $ xval yval           # move to <float,float>\n");
        fprintf(fp,"#  ) xval yval           # draw or continue to <float,float>\n");
        fprintf(fp,"#\n");
        fprintf(fp,"# Set colors: integer values in range 0 to 65536-1 using RGB color model\n");
        fprintf(fp,"#  ~ red green blue      # background color\n");
        fprintf(fp,"#  D red green blue      # fill color\n");
        fprintf(fp,"#  - red green blue      # pen color\n");
        fprintf(fp,"#\n");
        fprintf(fp,"#  0 width               # line thickness in user coordinates\n");
        fprintf(fp,"#  s xmin ymin xmax ymax # coordinate space\n");
        fprintf(fp,"#  e                     # erase\n");
        fprintf(fp,"#  o                     # open plot\n");
        fprintf(fp,"#  x                     # close plot\n");
        fprintf(fp,"#  NOTE: when using plot(1) if append metafiles attributes are carried on thru o/x\n");
        fprintf(fp,"#  O                     # restore state\n");
        fprintf(fp,"#  U                     # save state \n");
        fprintf(fp,"#  E                     # end path\n");
        fprintf(fp,"#  L saturation          # fill mode (0=off, 1->65535 is color saturation level\n");
        fprintf(fp,"#  Tjklabel              # j=lcr (left, center, right) k=tcbx (top, center, bottom, baseline)\n");
        fprintf(fp,"#  Ffontname             #\n");
        fprintf(fp,"#  7 fontsize            # font size \n");
        fprintf(fp,"#  ( textangle           # text path angle in degrees; ccw+, start angle 0-x\n");
        fprintf(fp,"#  Jjoinmode             # miter round bevel\n");
        fprintf(fp,"#  flinemode             # \n");
        fprintf(fp,"#GNU documentation is not clear on some issues, so experimented with plot(1)\n");
        fprintf(fp,"#and took conservative interpretations of what gets reset on a page clear or page end and so on\n");
        fprintf(fp,"#to minimize issues with appended meta files and plotting of single pages and such.\n");
        fprintf(fp,"#\n");
...
static int gnu_clear(void) {                    /* gnu_clear - Erase the plot */
   if (drawn) {
      putc('x',  fp);  /* end old page */
      putc('\n', fp);

      putc('o',  fp);  /* open new page */
      putc('\n', fp);
...
/******************************************************************************/
int _GNU_devcpy(void) {        /* copy the gnu plot device into vdevice.dev.  */
        vdevice.dev = gnudev;
        return(0);
}
/******************************************************************************/
