...
type(path)    :: file
   ...
   call unit_check('branch', file%branch().eq.'/home/user', msg=msg(file%name,'branch',file%branch()))
...
type(path)    :: file
   ...
   call unit_check('leaf', file%leaf().eq.'file.txt', msg=msg(file%name,'leaf',file%leaf()))
...
type(path)    :: file
   ...
   call unit_check('stem', file%stem().eq.'file', msg=msg(file%name,'stem',file%stem()))
...
type(path)    :: file
   ...
   call unit_check('bud', file%bud().eq.'.txt', msg=msg(file%name,'bud',file%bud()))
...
type(path)    :: file
   ...
   call unit_check('path_exists', file%exists(), msg=msg(file%name,file%exists()))
   file%name='_notthere_/_notthere_'
   call unit_check('path_exists', .not.file%exists(), msg=msg(file%name,file%exists()))
...
integer                      :: ierr
   ...
   call unit_check('path_isdir', ierr.eq.0, msg=msg('getting current directory',dirname))
   call file%init(dirname)
   call unit_check('path_isdir', file%is_dir(), msg=msg('current directory',file%name,'isdir',file%is_dir()))
...
integer                      :: ierr
   ...
   call file%init('.')
   call unit_check('path_realpath', file%realpath().eq.dirname, msg=msg('pathname=',file%name,'=>',file%realpath(),'cwd=',dirname))
