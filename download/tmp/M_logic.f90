...
!  if variable is not zero.
   ...
   call unit_check('cond',icheck.eq.0,msg=msg('CALCULATOR VARIABLE check_count should be zero',icheck))
   call unit_check('cond',nest_level.eq.0,msg=msg('nesting level should be zero at end of file'))
!-----------------------------------------------------------------------------------------------------------------------------------
   ...
   if(.not.allocated(text))then
      call unit_check_bad('cond',msg=msg('failed to load file of expected results')) ! flag that got unexpected ending
   else
      do i=1,size(text)
         call unit_check('cond',text(i).eq.expected(i),msg=msg('EXPECTED:',expected(i),'GOT:',text(i)))
      enddo
