...
! ____________________________________________________________________
      character (len=*), parameter :: zsccs = "@(#)splitidnt.f90      1.5     98/10/24 Michel Olagnon, Phil Garnatz"
      character (len=*), parameter :: zvers = "@(#) splitidnt.f90     V-1.1 98/10/24 Michel Olagnon, Phil Garnatz"
      character (len=*), parameter :: zusg  = "( usage: f90split < largefile [ > list_file ] )"
...
! ____________________________________________________________________
character (len=26), parameter :: zupc_bug="ABCDEFGHIJKLMNOPQRSTUVWXYZ"
character (len=26), parameter :: zlwc_bug="abcdefghijklmnopqrstuvwxyz"
! ____________________________________________________________________
!
!   Modified to not do upper case to embedded strings (pgg 21.11.94)
      ...
      do istr = 1, lstr
         if (toggle) then
            ...
            irnk = index (zlwc_bug, zstr (istr:istr))
            if (irnk > 0) then
               zstr (istr:istr) = zupc_bug(irnk:irnk) !!!!!!!!!!!!!!BUG IN G95 !!!!!!!!!!!!!!!!!
            endif
         else
...
!
      lstr = len_trim (zstr)
      do istr = 1, lstr
         irnk = index (zupc_bug, zstr (istr:istr))
         if (irnk > 0) then
            zstr (istr:istr) = zlwc_bug(irnk:irnk)   !!!!!!!!!!!!!!BUG IN G95 !!!!!!!!!!!!!!!!!
         endif
      enddo
...
!~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
! /tmp/CCALL/ccvgdp2h.o(.text+0x4342): In function `raicas_': /tmp/CCALL/f90split.2660.f90:1372: undefined reference to `splitprms_MP_zupc'
! /tmp/CCALL/ccvgdp2h.o(.text+0x4435): In function `lwrcas_': /tmp/CCALL/f90split.2660.f90:1390: undefined reference to `splitprms_MP_zlwc'
