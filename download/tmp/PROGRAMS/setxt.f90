...
'@(#)COMPILED:       Mon, Mar 25th, 2019 12:14:21 AM>',&
'']
...
character(len=:),allocatable   :: cache
   ...
   call kracken('setxt','-rows -1 -cols -1 -down -999999 -right -999999 &
        & -fn "#N#" -fsize -1 &
        & -bg "#N#" -fg "#N#" -cr "#N#" -cn "#N#"&
        & -iconstate "#N#" &
        & -title "#N#" -name "#N#" -nt "#N#" &
        & -help .f. -version .f. -debug F -alias F -xrdb "#N#" -o ')
   call help_usage(lget('setxt_help'))                              ! if -help option is present, display help text and exit
   call help_version(lget('setxt_version'))                         ! if -version option is present, display version text and exit
   if(lget('setxt_alias'))then
      call alias()
      stop
   endif
   debug=lget('setxt_version')
   background = trim(sget('setxt_bg'))
   foreground = trim(sget('setxt_fg'))
   cursor = trim(sget('setxt_cr'))

   array=sgets('setxt_oo')
   do i=1,size(array)
   ...
   enddo
!===================================================================================================================================
   label=trim(sget('setxt_title'))
   if(label.ne.'#N#')then
      call xterm_labels('title',label)
      noscreen=.true.
   endif
   label=trim(sget('setxt_name'))
   if(label.ne.'#N#')then
      call xterm_labels('name',label)
      noscreen=.true.
   endif
   label=trim(sget('setxt_nt'))
   if(label.ne.'#N#')then
      call xterm_labels('nt',label)
      noscreen=.true.
   endif
!===================================================================================================================================
   cache=trim(sget('setxt_cn'))
   if(cache.eq.'#N#')then                                 ! if -cn is not specified
   elseif(cache.eq.'')then                                ! if -cn with no parameters list first sixteen pen values
      do i=0,15
   ...
   elseif(all(isdigit(switch(cache)).or.isspace(switch(cache))))then ! if all non-space characters are digits list those pen numbers
      ...
      enddo
      noscreen=.true.
   else                                                   ! assume this is a string of form NNNN #RRGGBB|name|rgb:/RR/GG/BB ...
...
!===================================================================================================================================
POSITION: block
   num_right= iget('setxt_right')
   num_down= iget('setxt_down')

   if(sget('setxt_right').eq.'')then
...
!===================================================================================================================================
   if(noscreen)then                                                 ! if no options were specified display screen panel
      stop
   endif
!===================================================================================================================================
   page_ptr=>page_pd
...
'alias 80=''setxt 80''                                                           ',&
'alias 132=''setxt 132''                                                         ',&
'alias lower=''setxt lower''                                                     ',&
'alias raise=''setxt raise''                                                     ',&
'alias icon=''setxt iconify''                                                    ',&
'alias unicon=''setxt uniconify''                                                ',&
'alias 0=''setxt -fn 0''                                                         ',&
'alias 1=''setxt -fn 1''                                                         ',&
'alias 2=''setxt -fn 2''                                                         ',&
'alias 3=''setxt -fn 3''                                                         ',&
...
'alias 7=''setxt -fn 7''                                                         ',&
'alias 8=''setxt -fn 8''                                                         ',&
'alias small=''setxt -rows 24 -cols 80''                                         ',&
'alias full=''setxt maximize''                                                   ',&
'alias fullback=''setxt restore''                                                ',&
'################################################################################',&
...
'"                                                                               ',&
'alias green=''setxt -rows 24 -cols 80 -fn huge -bg green4 -fg yellow -cr red''  ',&
'alias brown=''setxt -fn huge -bg brown4 -fg white -cr red''                     ',&
'################################################################################',&
'function ID(){                                                                  ',&
...
'   1) setxt -bg $1        ;;                                                    ',&
'   2) setxt -bg $1 -fg $2 ;;                                                    ',&
'   3) setxt -bg $1 -fg $2 -cr $3;;                                              ',&
'   *) setxt -bg $1 -fg $2 -cr $3;;                                              ',&
'   esac                                                                         ',&
'}                                                                               ',&
'################################################################################',&
'function trykolor(){                                                            ',&
